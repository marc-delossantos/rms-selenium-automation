...
async function IT_018() {
    ...
    try {
        ...
        await resizeScreen(driver);

        const screenshotPath = await takeScreenshot(driver, 'test_018');
        await writeResult('RPM:InterActiv_test_IT_018', 'PASS', screenshotPath);
        
    } catch (error) {
        console.error('test failed:', error.message);
        let screenshotPath;
        if (driver) {
            screenshotPath = await takeScreenshot(driver, 'test_018');
            await writeResult('RPM:InterActiv_test_IT_018', 'FAILED', screenshotPath, error.message); // <-- Include screenshot for FAIL
        }
    }finally {
        if (driver) {
