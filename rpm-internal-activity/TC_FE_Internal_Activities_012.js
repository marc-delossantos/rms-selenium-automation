...
async function IT_012() {
    ...
    try {
        ...
        const selectSTAT1 = await driver.findElement(By.xpath(`/html/body/div/div/div/div[2]/div/div/div[2]/div[1]/div[1]/div/div[4]/div/div/div[2]/div[1]/span`))
        await selectSTAT1.click();
        await driver.sleep(500);
        
        await tableCheck(driver,RPM_InterActiv.TBL.table,'Status','Active');

        const screenshotPath = await takeScreenshot(driver, 'test_012');
        await writeResult('RPM:InterActiv_test_IT_012', 'PASS', screenshotPath);
        
    } catch (error) {
        console.error('test failed:', error.message);
        let screenshotPath;
        if (driver) {
            screenshotPath = await takeScreenshot(driver, 'test_012');
            await writeResult('RPM:InterActiv_test_IT_012', 'FAILED', screenshotPath, error.message); // <-- Include screenshot for FAIL
        }
    }finally {
        if (driver) {
