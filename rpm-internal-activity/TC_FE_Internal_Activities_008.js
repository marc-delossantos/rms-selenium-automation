...
async function IT_008() {
    ...
    try {
        ...
        const STATOptions = await driver.findElements(RPM_InterActiv.DRPDWN.STATOptions);

        const expectedSTATOptions = data.dropdown.status;
        const actualSTATOptions = [];
        for (const option of STATOptions) {
            ...
            actualSTATOptions.push(...items); // push each item separately
        }

        assert.deepStrictEqual(actualSTATOptions, expectedSTATOptions);
        console.log('Dropdown options match expected values!');

        const screenshotPath = await takeScreenshot(driver, 'test_008');
        await writeResult('RPM:InterActiv_test_IT_008', 'PASS', screenshotPath);
        
    } catch (error) {
        console.error('test failed:', error.message);
        let screenshotPath;
        if (driver) {
            screenshotPath = await takeScreenshot(driver, 'test_008');
            await writeResult('RPM:InterActiv_test_IT_008', 'FAILED', screenshotPath, error.message); // <-- Include screenshot for FAIL
        }
    }finally {
        if (driver) {
