...
async function IT_011() {
    ...
    try {
        ...
        const selectCAT1 = await driver.findElement(By.xpath(`/html/body/div/div/div/div[2]/div/div/div[2]/div[1]/div[1]/div/div[3]/div/div/div[2]/div[1]/span`))
        await selectCAT1.click();
        await driver.sleep(500);
        
        await tableCheck(driver,RPM_InterActiv.TBL.table,'Category','Internal Project');

        const screenshotPath = await takeScreenshot(driver, 'test_011');
        await writeResult('RPM:InterActiv_test_IT_011', 'PASS', screenshotPath);
        
    } catch (error) {
        console.error('test failed:', error.message);
        let screenshotPath;
        if (driver) {
            screenshotPath = await takeScreenshot(driver, 'test_011');
            await writeResult('RPM:InterActiv_test_IT_011', 'FAILED', screenshotPath, error.message); // <-- Include screenshot for FAIL
        }
    }finally {
        if (driver) {
